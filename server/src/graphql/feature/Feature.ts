  ...
  definition (t) {
    ...
    t.nonNull.list.nonNull.field('controls', {
      ...
      resolve (parent, _, context) {
        return context.prisma.control
          .findMany({ where: { featureId: parent.id } })
      }
    })
    t.id('partId')
    t.nonNull.field('part', { // 1
      type: 'Part',
      resolve (parent, _, context) { // 2
        return context.prisma.part
          .findUnique({ where: { id: parent.partId } })
      }
    })
  }
})
